...
class double_power_point_spp02g extends ZigBeeDriver {
    async onInit() {
        ...
        this._leftSocketOn = this.homey.flow.getDeviceTriggerCard("left-socket-on");
        this._leftSocketOff = this.homey.flow.getDeviceTriggerCard("left-socket-off");
        this._rightSocketOn = this.homey.flow.getDeviceTriggerCard("right-socket-on");
        this._rightSocketOff = this.homey.flow.getDeviceTriggerCard("right-socket-off");

        // left_socket
        this.homey.flow.getActionCard('left_socket_on').registerRunListener(async (args, state) => {
          await args.device.zclNode.endpoints[1].clusters[CLUSTER.ON_OFF.NAME].setOn();
        });
        this.homey.flow.getActionCard('left_socket_off').registerRunListener(async (args, state) => {
          await args.device.zclNode.endpoints[1].clusters[CLUSTER.ON_OFF.NAME].setOff();
        });
        // right_socket
        this.homey.flow.getActionCard('right_socket_on').registerRunListener(async (args, state) => {
          await args.device.zclNode.endpoints[2].clusters[CLUSTER.ON_OFF.NAME].setOn();
        });
        this.homey.flow.getActionCard('right_socket_off').registerRunListener(async (args, state) => {
